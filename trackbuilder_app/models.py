...
class BullheadChairs(models.Model):
    ...
    def clean(self):
        super().clean()
        # Additional business rules validation, if necessary
        # Example: Ensure plinth thickness isn't unreasonably large
...
class BullheadJawSection(models.Model):
    ...
    chair = models.ForeignKey(BullheadChairs, on_delete=models.CASCADE, related_name="jaw_sections")
...
class BullheadKey(models.Model):
    ...
    chair = models.ForeignKey(BullheadChairs, on_delete=models.CASCADE, related_name="keys")
...
class BullheadBossAndFerrule(models.Model):
    ...
    chair = models.ForeignKey(BullheadChairs, on_delete=models.CASCADE, related_name="components")
...
class Track(models.Model):
    ...
    gauge = models.ForeignKey(Gauge, on_delete=models.CASCADE, related_name="tracks")
    ...
    chair_alignment = models.CharField(max_length=20, choices=CHAIR_ALIGNMENT_CHOICES, help_text="Chair alignment pattern")
    is_straight = models.BooleanField(default=True, help_text="Is the track straight?")
    kerf_adjustment = models.FloatField(blank=True, null=True, help_text="Adjustment for material kerf or tolerances")
    flange_width = models.FloatField(blank=True, null=True, help_text="Width of the flange in inches")
    flange_depth = models.FloatField(blank=True, null=True, help_text="Depth of the flange in inches")
    chair_type = models.ForeignKey(BullheadChairs, on_delete=models.CASCADE, related_name="tracks")
...
class Timber(models.Model):
    track = models.ForeignKey(Track, on_delete=models.CASCADE, related_name="timbers")
    length = models.FloatField()
    width = models.FloatField()
    depth = models.FloatField()
    position = models.FloatField()
    chair = models.ForeignKey(BullheadChairs, on_delete=models.CASCADE, related_name="timbers")
...
class STLDownloadLog(models.Model):
    ...
    chair = models.ForeignKey(BullheadChairs, on_delete=models.SET_NULL, null=True, blank=True, related_name="downloads")
    track = models.ForeignKey(Track, on_delete=models.SET_NULL, null=True, blank=True, related_name="downloads")
    timestamp = models.DateTimeField(auto_now_add=True)
    file_path = models.CharField(max_length=255, help_text="Path to the downloaded STL file")
